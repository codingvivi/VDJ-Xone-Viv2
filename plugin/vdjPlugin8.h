...
#define WIN32_LEAN_AND_MEAN  // For preventing linking errors with
                             // DllGetClassObject
#include <windows.h>
#define VDJ_WIN
#if (defined(_M_X64) || defined(__amd64__))
#define VDJ_IS_WIN64  // if you need special code for the 64bit version of
                      // VirtualDJ, use this
#endif
...
typedef SInt32 HRESULT;  // Error code shit Windows is hopped up on
...
struct TVdjPluginInterface8
{
  ...
  void *ImageBuffer;
  int ImageSize;
  // Type==VDJINTERFACE_DIALOG. HWND returned by CreateDialog or CreateWindow on
  // Windows, or NSWindow pointer on mac
  VDJ_WINDOW hWnd;
};
...
//////////////////////////////////////////////////////////////////////////
// Internal structures
// override whatever insides functions with virtual have
struct IVdjCallbacks8
{
  ...
  virtual HRESULT GetStringInfo(const char *command, void *result,
                                int size) = 0;
...
struct IVdjCallbacks8;  // not sure why forward dec is afterwards
...
class IVdjPlugin8
{
 public:
  ...
  // if type=VDJPARAM_CUSTOM or VDJPARAM_STRING, parameterSize must be set to
  // sizeof(*parameter)
  HRESULT DeclareParameterButton(int *parameter, int id, const char *name,
                                 const char *shortName)
  {
    return cb->DeclareParameter(parameter, VDJPARAM_BUTTON, id, name, shortName,
                                0.0f);
  }
  HRESULT DeclareParameterSlider(float *parameter, int id, const char *name,
                                 const char *shortName, float defaultvalue)
  {
    return cb->DeclareParameter(parameter, VDJPARAM_SLIDER, id, name, shortName,
                                defaultvalue);
  }
  HRESULT DeclareParameterSwitch(int *parameter, int id, const char *name,
                                 const char *shortName, bool defaultvalue)
  {
    return cb->DeclareParameter(parameter, VDJPARAM_SWITCH, id, name, shortName,
                                (float)defaultvalue);
  }
  HRESULT DeclareParameterString(char *parameter, int id, const char *name,
                                 const char *shortName, int parameterSize)
  {
    return cb->DeclareParameter(parameter, VDJPARAM_STRING, id, name, shortName,
                                (float)parameterSize);
  }
  HRESULT DeclareParameterCustom(void *parameter, int id, const char *name,
                                 const char *shortName, int parameterSize)
  {
    return cb->DeclareParameter(parameter, VDJPARAM_CUSTOM, id, name, shortName,
                                (float)parameterSize);
  }
  HRESULT DeclareParameterRadio(int *parameter, int id, const char *name,
                                const char *shortName, float defaultvalue)
  {
    return cb->DeclareParameter(parameter, VDJPARAM_RADIO, id, name, shortName,
                                (float)defaultvalue);
  }
  HRESULT DeclareParameterCommand(char *parameter, int id, const char *name,
                                  const char *shortName, int parameterSize)
  {
    return cb->DeclareParameter(parameter, VDJPARAM_COMMAND, id, name,
                                shortName, (float)parameterSize);
  }

  // OnParameter will be called each time a parameter is changed from within
  // VirtualDJ
  virtual HRESULT VDJ_API OnParameter(int id) { return S_OK; }
  // OnGetParameterString will be called each time the string label of a
  // parameter is requested by VirtualDJ
  virtual HRESULT VDJ_API OnGetParameterString(int id, char *outParam,
                                               int outParamSize)
  {
    return E_NOTIMPL;
  };
  ...
  // Fill the VDJ_WINDOW  or xml/bitmap info in the passed pluginInterface
  // structure, to define your own plugin window
  virtual HRESULT VDJ_API
  OnGetUserInterface(TVdjPluginInterface8 *pluginInterface)
  {
    return E_NOTIMPL;
  }
  ...
  // get info from VirtualDJ (as a value, or as a utf-8 string). You can also
  // use command = "setting 'xxx'" for VirtualDJ options
...
static const GUID CLSID_VdjPlugin8 = {
    ...
    0xF4F9,
    0x4DCD,
    {0xBD, 0x24, 0x29, 0x14, 0x12, 0xE9, 0x3B, 0x60}};
static const GUID IID_IVdjPluginBasic8 = {
    0xa1d90ea1,
    0x4d0d,
    0x42dd,
    {0xa4, 0xd0, 0xb8, 0xf3, 0x37, 0xb3, 0x21, 0xf1}};
#else
...
{
#endif
  VDJ_EXPORT HRESULT VDJ_API DllGetClassObject(const GUID &rclsid,
                                               const GUID &riid,
                                               void **ppObject);
#ifdef __cplusplus
}
